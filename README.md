...
This builds the all-in-one jar file, `pyjinn-app-0.10-all.jar`:

```
% ls -lh app/build/libs/pyjinn-app-0.10-all.jar
-rw-r--r--@ 1 maxuser  staff   1.1M Aug 25 23:19 app/build/libs/pyjinn-app-0.10-all.jar
```

Run the standalone interpreter using `java -jar ...`, e.g. on Mac, Linux, or with WSL on Windows:

```
$ echo 'print([x for x in range(10) if x > 2])' |java -jar app/build/libs/pyjinn-app-0.10-all.jar
[3, 4, 5, 6, 7, 8, 9]
```
...
```
% ls -lh interpreter/build/libs/pyjinn-lib-0.10-all.jar
-rw-r--r--@ 1 maxuser  staff   926K Aug 25 23:10 interpreter/build/libs/pyjinn-lib-0.10-all.jar
```
...
```
$ java -jar app/build/libs/pyjinn-app-0.10-all.jar << 'EOF'
...
```
~/.m2/repository/org/pyjinn/pyjinn-lib/0.10/pyjinn-lib-0.10-all.jar
```
...
dependencies {
    implementation 'org.pyjinn:pyjinn-lib:0.10:all'
}
```